...
const TABS = [
  ...
  { id: "plans", label: "Manage Plans", icon: "fa-layer-group" },
...
export default function BillingTabs({ generalContent, plansContent, activityContent }: Props) {
  const searchParams = useSearchParams();

  const urlTab = searchParams.get("section");
  const resolvedDefault = TABS.find((t) => t.id === urlTab) ? (urlTab as TabId) : "general";
  ...
  const switchTab = useCallback((tabId: TabId) => {
    ...
    const url = `/dashboard/admin/billing${qs ? `?${qs}` : ""}`;
