...
export default function InvitesManager({ invites: initialInvites }: { invites: Invite[] }) {
  ...
  const handleSend = () => {
    ...
    startTransition(async () => {
      const res = await sendAgencyInviteAction(email.trim(), name.trim() || undefined);
      if (res.ok) {
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full px-4 py-2.5 rounded-xl bg-surface-container-high/40 border border-outline-variant/10 text-on-surface text-sm placeholder:text-on-surface-variant/30 focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary/30 transition-all"
